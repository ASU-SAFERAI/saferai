...
from deepeval.metrics.dag.templates import NonBinaryJudgementTemplate
from deepeval.metrics.dag.schema import NonBinaryJudgementVerdict
...
def nonbinary_judgment_node_batch_generate(model: DeepEvalClient,
                                          nodes: List[NonBinaryJudgementNode],
    ...
    for node, metric, test_case in zip(nodes, metrics, test_cases):
        ...
        prompt = NonBinaryJudgementTemplate.generate_non_binary_verdict(
            criteria=node.criteria, text=text, options=node._verdict_options
        )
    ...
    DynamicSchema = nodes[0]._verdict_schema
    if prompts:
        model.request_dict.metric_phase = f"nonbinary_judgments_depth_{depth}"
        responses = model.batch_generate(
            prompts=prompts,
            schema=DynamicSchema
        )
        for (node, metric), response in zip(valid_items, responses):
            if isinstance(response, DynamicSchema):
                node._verdict = response
            else:
                node._verdict = DynamicSchema(
                    verdict="Error: Invalid response", reason="Unable to parse response."
                )
