...
from deepeval.metrics.dag.templates import TaskNodeTemplate
...
def task_node_batch_generate(model: DeepEvalClient,
                             nodes: List[TaskNode], metrics: Sequence[BaseMetric],
    ...
    if prompts:
        model.request_dict.metric_phase = "task_nodes"
        responses = model.batch_generate(
            prompts=prompts,
            schema=TaskNodeOutput
        )
        for (node, metric), response in zip(valid_items, responses):
            if isinstance(response, TaskNodeOutput):
                node._output = response.output
            else:
                # Fallback for invalid response (mimicking original's trimAndLoadJson logic if needed)
                node._output = TaskNodeOutput(output="Error: Unable to parse response.").output
